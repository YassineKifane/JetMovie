...
class MovieRepositoryImpl(
    ...
    override fun searchMovies(query: String): Flow<Response<List<Movie>>> = flow {
        emit(Response.Loading())
        val searchDto = movieApiService.searchMovies(query) // Retrofit endpoint à créer, qui appelle TMDB Search API
        emit(Response.Success(apiMapper.mapToDomain(searchDto)))
    }.catch { e ->
        emit(Response.Error(e))
    }
}