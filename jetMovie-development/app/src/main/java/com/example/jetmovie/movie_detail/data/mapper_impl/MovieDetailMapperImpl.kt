...
import com.example.jetmovie.movie_detail.domain.models.Videos
import com.example.jetmovie.movie_detail.domain.models.Video
...
class MovieDetailMapperImpl : ApiMapper<MovieDetail, MovieDetailDto> {
    override fun mapToDomain(apiDto: MovieDetailDto): MovieDetail {
        return MovieDetail(
            ...
            videos = apiDto.videos?.let { videosDto ->
                Videos(
                    results = videosDto.results.map { videoDto ->
                        Video(
                            id = videoDto.id,
                            key = videoDto.key,
                            site = videoDto.site,
